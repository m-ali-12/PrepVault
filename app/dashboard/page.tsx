    ...
    <StudentShell>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/exams" className="text-sm text-cyan-300 font-bold glass px-4 py-2 rounded-xl">
            Switch Exam ↗
          </Link>
        </div>
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-6">
          <section>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              ...
              <div className="glass rounded-2xl p-5">
                <div className="text-slate-400 text-sm">Total Levels</div>
                <div className="text-3xl font-black">{levels.length || '—'}</div>
              </div>
              <div className="glass rounded-2xl p-5">
                <div className="text-slate-400 text-sm">Questions</div>
                <div className="text-3xl font-black">{levels.reduce((s: number, l: any) => s + (l.verified_questions || 0), 0) || '—'}</div>
              </div>
            </div>
            ...
            {levels.length === 0 ? (
              <div className="glass rounded-3xl p-8 text-center">
                ...
                <p className="text-slate-400 mt-2">Questions and study material for {meta.name} are being verified and added. In the meantime, try HAT which has full sample content.</p>
              ...
              <div className="space-y-3">
                {levels.map((l: any, i: number) => (
                  <div key={l.id} className="glass rounded-2xl p-5 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl font-black ${i === 0 ? 'bg-cyan-400 text-slate-950' : 'bg-white/10'}`}>
                        {l.level_no}
                      </div>
                      <div>
                        <div className="text-xs text-slate-400 mb-1">
                          {i === 0 ? '🟢 Unlocked' : '🔒 Unlocks after Level ' + (i)} • {l.verified_questions || 0} questions
                        </div>
          ...
          <aside className="space-y-4">
            ...
            <div className="glass rounded-3xl p-6">
              ...
              <p className="text-slate-400 text-sm mt-2">
                Ask any {meta.name} question and get a step-by-step explanation. Chat history is saved to your account.
              </p>
              <Link href="/ai-tutor" className="mt-4 inline-block w-full text-center rounded-xl neon px-5 py-3 font-bold">
                Open AI Tutor
              </Link>
            </div>

            <div className="glass rounded-3xl p-6">
              <h3 className="text-lg font-bold">⚡ Anti-Cheat</h3>
              <p className="text-slate-400 text-sm mt-2">
                Tests run in fullscreen mode. Tab switching is tracked. Questions are randomized every attempt.
              </p>
            </div>

            <div className="glass rounded-3xl p-6">
              <h3 className="text-lg font-bold">📊 How levels work</h3>
              <div className="mt-3 space-y-2 text-sm text-slate-300">
                <div className="flex gap-2"><span className="text-green-400">✓</span> Study the material</div>
                <div className="flex gap-2"><span className="text-cyan-400">✓</span> Take the level test</div>
                <div className="flex gap-2"><span className="text-yellow-400">✓</span> Score 85% or above</div>
                <div className="flex gap-2"><span className="text-purple-400">✓</span> Next level unlocks</div>
                <div className="flex gap-2"><span className="text-red-400">✓</span> Below 85%? Retest with new questions</div>
              </div>
            </div>
          </aside>
        </div>
